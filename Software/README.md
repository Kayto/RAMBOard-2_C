...
| Name  | Description |
|----------|:-------------|
| **[WHEREISMYRAM.bas](whereismyram/whereismyram.bas)** | This BASIC program runs through and checks 2K blocks of RAM. |
| **[1541RAMREAD.bas](1541ramread/1541RAMREAD.bas)** | This BASIC program list the contents of a particular area of drive ROM/RAM. Useful to check whether the code you want is actually there. |
...
| **[Parallel Collatz Demo](https://github.com/Kayto/RAMBOard-2_C/tree/main/Software/collatz)** | True parallel computation between the C64 and 1541 drive, both running the Collatz conjecture (3n+1 problem). In parallel mode the C64 computes odd numbers and the 1541 computes even numbers (N=1–3000) simultaneously, using the RAMBOard to buffer 1500 results. Includes solo mode for baseline comparison — parallel mode is roughly 2× faster. Features a paginated sequence viewer. |
| **[Parallel Prime Sieve Demo](https://github.com/Kayto/RAMBOard-2_C/tree/main/Software/primes)** | True parallel computation between the C64 and 1541 drive for primality testing by trial division across N=1–3000. In parallel mode the C64 tests N=1–1500 and the 1541 tests N=1501–3000 simultaneously, using the RAMBOard to buffer results. Includes solo mode for baseline comparison — parallel mode is roughly 2× faster. Features a paginated result viewer showing primes and which CPU found them. |
